 ...
 */
        ...
        done_stop_sign_overlap_id(),

        "");
        EXPECT_EQ(stop_sign_status
        .

        wait_for_obstacle_id_size(),
...
ENABLE_RULE(TrafficRuleConfig::CROSSWALK,
false);
ENABLE_RULE(TrafficRuleConfig::KEEP_CLEAR,
true);
ENABLE_RULE(TrafficRuleConfig::TRAFFIC_LIGHT,
false);
...
auto *crosswalk_status = injector_->planning_context()
        ...
        ->mutable_crosswalk();
EXPECT_EQ("2832", crosswalk_status->

crosswalk_id()

);
EXPECT_EQ(1, crosswalk_status->

stop_time_size()

);
EXPECT_EQ("11652", crosswalk_status->stop_time(0).
...
set_stop_timestamp_sec (Clock::NowInSeconds()
...
ENABLE_RULE(TrafficRuleConfig::CROSSWALK,
false);
ENABLE_RULE(TrafficRuleConfig::DESTINATION,
true);
ENABLE_RULE(TrafficRuleConfig::KEEP_CLEAR,
false);
ENABLE_RULE(TrafficRuleConfig::TRAFFIC_LIGHT,

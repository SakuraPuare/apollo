...
namespace apollo {
    namespace planning {
        ...
        ACHECK(!end_conditions.

        empty()

        );

        ptr_trajectory_bundle->
        reserve(ptr_trajectory_bundle
        ->
        ...
        ptr_trajectory1d->
        set_target_velocity(end_condition
        .first[1]);
        ptr_trajectory1d->
        set_target_time(end_condition
        .second);
        ptr_trajectory_bundle->
        push_back(ptr_trajectory1d);
    }
}
...
ptr_trajectory1d->
set_target_position(end_condition
.first[0]);
ptr_trajectory1d->
set_target_velocity(end_condition
.first[1]);

...
// ObsTEdge contains: (is_starting_t, t, s_min, s_max, obs_id).
        ...
        ADEBUG << "There are " << obs_t_edges_.

        size()

        << " t-edges.";
        std::vector <ObsTEdge> new_t_edges;
        while (obs_t_edges_idx_ < static_cast
        <int>(obs_t_edges_
        .
        ...
        <= t) {
        if (
        std::get<0>(obs_t_edges_[obs_t_edges_idx_])
        == 0 &&
        std::get<1>(obs_t_edges_[obs_t_edges_idx_])
        == t) {
        break;
    ...
    std::get<1>(obs_t_edges_[obs_t_edges_idx_]);
    new_t_edges.
    push_back(obs_t_edges_[obs_t_edges_idx_]);
    ++
    obs_t_edges_idx_;
}
...
for (
const auto &obs_t_edge
: new_t_edges) {
if (
std::get<0>(obs_t_edge)
== 0) {
ADEBUG << "Obstacle id: " <<
std::get<4>(obs_t_edge)
<< " is leaving st-graph.";
if (obs_id_to_decision_.
count(std::get<4>(obs_t_edge)
) != 0) {
obs_id_to_decision_.
erase(std::get<4>(obs_t_edge)
);
}
}
}
...
for (
const auto &obs_id_to_decision_pair
: obs_id_to_decision_) {
...
obs_id_to_remove.
push_back(obs_id_to_decision_pair
.first);
}
}
for (
...
if (obs_id_to_alternative_st_boundary_.
count(obs_id)
> 0) {
...
obs_ptr->
set_path_st_boundary(obs_id_to_alternative_st_boundary_[obs_id]);
}
}
...
double obs_s_max = 0.0;
obs_st_boundary.
GetBoundarySRange(t, &obs_s_max, &obs_s_min
);
if (obs_decision.
...
auto obs_t_edge
: new_t_edges) {
ADEBUG << "For obstacle id: " <<
std::get<4>(obs_t_edge)
<< ", its s-range = [" <<
std::get<2>(obs_t_edge)
<< ", "
<<

...
EXPECT_DOUBLE_EQ(obj_value,
1443.3600000000008) << "eval_f: " <<
obj_value;
EXPECT_TRUE(res);
}
...
) << "iRow differ at index " <<
i;
EXPECT_EQ(j_col_ser[i], j_col_par[i]
) << "iCol differ at index " <<
i;
}
...
std::fill_n(grad_f_adolc, n,
0.0);
std::fill_n(grad_f_hand, n,
0.0);

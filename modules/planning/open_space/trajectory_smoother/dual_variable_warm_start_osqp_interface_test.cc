...
namespace apollo {
    ...
    EXPECT_TRUE(res);
    ptop_->
    get_optimization_results(&l_warm_up, &n_warm_up
    );
    ...
    app->Options()->SetNumericValue("tol", ipopt_config_tmp.
    ...
    app->Options()->SetStringValue("recalc_y", ipopt_config_tmp.
    ...
    app->
    OptimizeTNLP(problem);

    // Retrieve some statistics about the solve
    pt_gt_->
    get_optimization_results(&l_warm_up_gt, &n_warm_up_gt
    );

    // compare ipopt_qp and osqp results
    for (
    int r = 0;
    r<obstacles_edges_sum;
    ++r) {
    for (
    int c = 0;
    c < static_cast
    <int>(horizon_)
    + 1; ++c) {
    EXPECT_NEAR(l_warm_up(r, c), l_warm_up_gt(r, c),
    1e-6)
    << "r: " << r << ", c: " <<
    c;
}
...
<int>(horizon_)
+ 1; ++c) {
EXPECT_NEAR(n_warm_up(r, c), n_warm_up_gt(r, c),
1e-6)
<< "r: " << r << ",c: " <<
c;
}

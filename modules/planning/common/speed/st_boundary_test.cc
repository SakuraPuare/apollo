...
namespace apollo {
    namespace planning {
        ...
        point_pairs.
        emplace_back(lower_points[0], upper_points[0]
        );
        point_pairs.
        emplace_back(lower_points[1], upper_points[1]
        );
        ...
        boundary_type(), STBoundary::BoundaryType::UNKNOWN

        );
        EXPECT_DOUBLE_EQ(0.0, boundary.

        min_s()

        );
        EXPECT_DOUBLE_EQ(5.0, boundary.
        ...
        EXPECT_DOUBLE_EQ(10.0, boundary.
    ...
    boundary.
    SetBoundaryType(STBoundary::BoundaryType::YIELD);
    double t = -10.0;
    ...
    .
    GetUnblockSRange(t, &high, &low
    ));
    EXPECT_DOUBLE_EQ(low,
    0.0);
    EXPECT_DOUBLE_EQ(high,
    200.0);
    EXPECT_FALSE(boundary
    .
    GetBoundarySRange(t, &high, &low
    ));
}
else {
EXPECT_TRUE(boundary
...
EXPECT_TRUE(
        boundary
.
GetIndexRange(lower_points,
-517957.08587679861, &left, &right));
EXPECT_EQ(left,
0);
EXPECT_EQ(right,
0);
...
points.
emplace_back(STPoint(0.0, 0.0), STPoint(1.0, 0.0)
);
points.
emplace_back(STPoint(0.1, 0.2), STPoint(1.1, 0.2)
);
points.
emplace_back(STPoint(0.2, 0.3), STPoint(1.2, 0.3)
);
points.
emplace_back(STPoint(0.3, 0.4), STPoint(1.3, 0.4)
);
points.
emplace_back(STPoint(0.4, 0.5), STPoint(1.4, 0.5)
);

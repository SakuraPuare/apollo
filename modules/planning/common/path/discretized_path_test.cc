...
namespace apollo {
    namespace planning {
        ...
        EXPECT_DOUBLE_EQ(discretized_path
        .

        Length(), std::sqrt(1.0 + 1.0)

        * 3.0);

        auto eval_p1 = discretized_path.Evaluate(0.0);
        EXPECT_DOUBLE_EQ(eval_p1
        .

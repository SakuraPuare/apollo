...
namespace apollo {
    namespace planning {
        ...
        size(), FLAGS_rtk_trajectory_forward

        );

        auto first_point = trajectory.begin();
        EXPECT_DOUBLE_EQ(first_point
        ->
    ...
    PlanOnReferenceLine(start_point, nullptr, &info
    ))
    .

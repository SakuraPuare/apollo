...
namespace apollo {
    namespace planning {
        ...
        ASSERT_TRUE(spline_constraint
        != nullptr);
        ASSERT_TRUE(spline_kernel
        != nullptr);
        ...
        spline_kernel->
        AddReferenceLineKernelMatrix(x_coord, fx_guide,
        0.4);
...
spline_constraint->
AddBoundary(x_coord, l_bound, u_bound
);
...
++i) {
t_knots[i] = static_cast
<double>(i)
* 0.1;
ft_knots[i] = t_knots[i] * slope +
intercept;
}

spline_kernel->
AddReferenceLineKernelMatrix(t_knots, ft_knots,
1);
EXPECT_TRUE(pg
.

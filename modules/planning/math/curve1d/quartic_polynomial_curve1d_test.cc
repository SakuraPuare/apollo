...
namespace apollo {
    namespace planning {
        ...
        EXPECT_NEAR(dx1, curve
        .Evaluate(1, param), 1e-8);
        EXPECT_NEAR(ddx1, curve
        .Evaluate(2, param), 1e-8);
...
EXPECT_NEAR(ddx1, curve
.Evaluate(2, param), 1e-8);
EXPECT_NEAR(x0, curve
.Evaluate(0, 0.0), 1e-8);
EXPECT_NEAR(dx0, curve
.Evaluate(1, 0.0), 1e-8);
EXPECT_NEAR(ddx0, curve
.Evaluate(2, 0.0), 1e-8);
}
}
...
quartic_curve.
IntegratedFromCubicCurve(cubic_curve,
0.0);
for (
double value = 0.0;
value < 5.1; value += 1) {
EXPECT_NEAR(quartic_curve
.Evaluate(1, value),
...
QuarticPolynomialCurve1d quartic_curve;
quartic_curve.
DerivedFromQuinticCurve(quintic_curve);
for (
double value = 0.0;
value < 5.1; value += 1) {
EXPECT_NEAR(quartic_curve

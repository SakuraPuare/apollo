...
namespace apollo {
    namespace planning {
        ...
        EXPECT_DOUBLE_EQ(153.87421245682503, reference_line_->
        ...
        common::util::uniform_slice(0.0, reference_line_->
    ...
    lateral_bound = 1e-6;
    smoother_->
    SetAnchorPoints(anchor_points);

    auto start = std::chrono::system_clock::now();

    EXPECT_TRUE(smoother_
    ->
    Smooth(*reference_line_, &smoothed_reference_line
    ));

    auto end = std::chrono::system_clock::now();
    std::chrono::duration<double> diff = end - start;
    std::cout << "Time to solver is " << diff.

    count()

    << " s\n";

    EXPECT_NEAR(152.5409, smoothed_reference_line.
